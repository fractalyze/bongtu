...
// critical; it reads top-down like the SPEC §6b endpoint list above.
export const routes: Route[] = [head, events, path, alarms, health];
  ...
  return (req: { url?: string; method?: string }, res: ServerResponse): void => {
    try {
      ...
      for (const route of routes) {
        ...
        const { status, body } = route.handle({ ix, params, query: url.searchParams });
