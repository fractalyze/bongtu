...
import type { Calldata } from "../../../../prover-cli/src/types.js";
import { deriveKeypair, commitment } from "../../../../sdk/src/note.js";
...
export function employerView(): HTMLElement {
  ...
  root.append(
    ...
    section("6 · Prove on the local GPU helper", [
      el("p", { class: "note", textContent: "Browser GPU proving is infeasible (1.24GB zkey + rabbitsnark). The honest PoC path: POST the assembled request to a local prover-cli helper on the employer's GPU box (apps/admin/prover-helper.ts), get calldata, submit from here." }),
