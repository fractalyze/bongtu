...
if [ -z "${E2E_DATABASE_URL:-}" ]; then
  if command -v "$DOCKER" >/dev/null 2>&1 && "$DOCKER" info >/dev/null 2>&1; then
    ...
    # The RECEIVE leg's indexer likewise owns its schema + cursor.
    "$DOCKER" exec "$PG_NAME" createdb -U postgres bongtu_receive || fail "createdb bongtu_receive failed"
    export E2E_RECEIVE_DATABASE_URL="postgres://postgres:postgres@127.0.0.1:${PG_PORT}/bongtu_receive"
  else
...
[ -n "${E2E_RECEIVE_DATABASE_URL:-}" ] || \
  fail "the receive leg needs its own Postgres database: export E2E_RECEIVE_DATABASE_URL (a different database on the same server is fine)"
