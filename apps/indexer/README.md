...
| `RECEIVE_FACTORY` | unset | ReceiveFactory address → the receive product lives (`POST /portal/announce` recorded against this factory's `addressOf`; `Swept` + `Announced` log ingest with chain-side backfill). Unset → `/portal/announce` 404s (one boot line) |
