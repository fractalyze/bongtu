...
consequence — checkable file-by-file against the pinned upstream checkout `2c8ce6b` (2026-07-16).
...
The **circuits are copied and modified** (below). `BongtuPool` shares no code with Zeto's
contracts: it is a purpose-built rewrite over storage Zeto's tree does not have, carrying the
flavor's three duties — proof binding (`authorityPublicKey` and `enabled` are injected by the
contract, never taken from calldata; deposit, 0-in, has no `enabled` signal), nullifier spend, and
non-repudiation envelope emission. Mechanics and events: [contracts.md](contracts.md#proof-binding).
...
([toolchain.md](toolchain.md#include-resolution-roots)). The membership- and envelope-bearing bases
are vendored into `circuits/lib/` and modified:
...
| `withdraw.circom` + `lib/check-nullifiers-value-imt-base.circom` | `lib/check-nullifiers-value-base.circom` | faithful derivation | SMT→IMT membership; value belt; zero-commitment guard; `GreaterEqThan(100)→(101)` completeness fix; authority envelope — 26-public base surface (the top-level wrapper appends public `recipient`, making the shipped surface `uint[27]`) |
...
(`encryptionNonce + i`) uniform across all five shapes. Shipped public surfaces:
[circuits.md](circuits.md#consumer-circuits-and-public-surfaces).

| bongtu file | parent (this repo) | deltas beyond the family-wide two |
|---|---|---|
| `lib/consumer-encrypt-outputs.circom` | `lib/encrypt-outputs-per-output-nonce.circom` (itself from Zeto `lib/encrypt-outputs.circom`) | the family's encryption gadget itself: ECDH re-targeted to per-output VIEW pubkeys (the spend key stays bound only by the commitment); no `kemBinding` output; NEW `viewTags[nOutputs]` — the canonical low 8 bits of a tagged Poseidon, decomposed with `Num2Bits_strict` because plain `Num2Bits(254)` admits a second decomposition (`tagField + p`) that flips the low byte; per-output nonce kept verbatim |
| `lib/consumer_deposit_imt_base.circom` | `lib/deposit_authority_imt_base.circom` | NEW per-output receiver ciphertexts + `viewTags` — a consumer deposit can mint directly to a third party who discovers it by scan (the enterprise deposit publishes an authority envelope only); stock `CheckHashes` and value-sum (`out`) checks kept verbatim |
| `lib/consumer_transfer_imt_small_base.circom` | `lib/anon_enc_nullifier_non_repudiation_imt_small_base.circom` | `ConsumerEncryptOutputs` replaces `EncryptOutputsPerOutputNonce`; NEW `viewTags` declared as the last output run; every input-side soundness constraint (enabled boolean, value belt, zero-commitment guard, `CheckPositive`, `CheckSum`, IMT membership) survives verbatim |
| `lib/consumer_withdraw_imt_base.circom` | `lib/check-nullifiers-value-imt-base.circom` | NEW receiver ciphertext + `viewTag` over the CHANGE note (the enterprise change is arbiter-recoverable; the consumer sender must recover change from chain scan alone); input-side belts and the `GreaterEqThan(101)` conservation check verbatim |
| `lib/consumer_disburse_imt_base.circom` | `lib/anon_enc_nullifier_non_repudiation_imt_base.circom` | per-output nonce is NEW at this shape — it structurally kills the two-time-pad class the enterprise disburse handles with assembly-time `assertDistinctOwnerPubkeys`; **extended** `disclosureHash` fold over `receiverCts[4B] ++ viewTags[B] ++ outputCommitments[B]` (6B elements; 1536 at B=256) replacing the enterprise `cts ++ cipherTextAuthority` preimage — the same commitment witnesses feed the fold, the note binding and the subtree builder; the enabled/value-belt pair stays absent exactly as in the parent (the module's `ZeroNullifier`-then-inject obligation) |
| `depositPriv` / `transferPriv` / `transfer10x2Priv` / `withdrawPriv` / `disbursePriv` / `disbursePriv256` `.circom` | the matching enterprise top (`deposit` / `transfer` / `transfer10x2` / `withdraw` / `disburse` / `disburse256` `.circom`) | thin sibling tops over the consumer bases; each header restates its full index layout. `withdrawPriv`'s wrapper appends ONE public input `recipient` (square-constraint calldata binding) so a stealth withdraw is relayer-submittable without the relayer redirecting funds. `disbursePriv256`'s 1536-element disclosure array rides the module's calldata bound by the extended fold — a PUBLIC indexer can verify it with no arbiter key |
...
For the two `lib/` files the mechanism is the include spelling — both are reached by bare name,
which has no counterpart at the `$ZETO` root, so only `-l lib` can resolve them (a stale untracked
copy may still sit at `$ZETO/lib/check-imt-proof.circom`, but its `lib/`-prefixed spelling never
matches a bare include, so it is inert —
