...
|---|---|
| **recipient** | own notes, from chain data alone, via the view identity (`viewPriv` + `kemDk`): viewTag filter → decapsulate → decrypt → leaf-match. The view identity can discover and decrypt but never spend |
...
| **sweep bot (operator key)** | the factory-owner EOA key + the token-gated attributed work feed | the same mapping, operator-side. Redirection-resistance rests on this key (the portal v1 concession): `sweep` is onlyOwner because the deposit proof binds no owner. On the **receive** pair it cannot OPEN what it mints — the depositPriv witness is built from the recipient's public triple, and the notes seal to keys only the recipient holds. The `funded` flag its work feed serves is chain-public transfer data attached operator-side; the sweep's proof-of-payment discipline (balance read, `pub[0]` binding) is unchanged by it |
