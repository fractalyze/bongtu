#!/usr/bin/env bash
# bongtu — the portalPriv-factory add-on deploy drill: a fresh Deploy.s.sol record
# (consumer module profile) taken through DeployPortalPriv.s.sol lands the factory,
...
PORT="${PORTAL_PRIV_DRILL_PORT:-8564}"
...
[ -z "$(jf portalPrivFactory)" ] || fail "fresh record must not carry a priv factory"

echo "== module-set precondition =="
mv "$MODS" "$MODS.aside"
(cd chains/evm && "$FORGE" script ../../deploy/forge/DeployPortalPriv.s.sol:DeployPortalPriv --rpc-url "$RPC" --broadcast --skip-simulation) >/dev/null 2>&1 && \
...
(cd chains/evm && "$FORGE" script ../../deploy/forge/DeployPortalPriv.s.sol:DeployPortalPriv --rpc-url "$RPC" --broadcast --skip-simulation) || fail "DeployPortalPriv failed"
F=$(jf portalPrivFactory); [ -n "$F" ] || fail "portalPrivFactory not recorded"
...
echo "== rerun refusal =="
(cd chains/evm && "$FORGE" script ../../deploy/forge/DeployPortalPriv.s.sol:DeployPortalPriv --rpc-url "$RPC" --broadcast --skip-simulation) >/dev/null 2>&1 && fail "rerun must refuse" || echo "   PASS: rerun refused"
echo "PORTAL PRIV DEPLOY DRILL: PASS"
