...
// treasury wording for a failed run (treasuryErrorMessage) baked into the machine,
 ...
 *  (treasuryErrorMessage — the one per-app delta the old copies carried), keeping the
...
export class ActionMachine<O extends ActionResult> extends SharedActionMachine<O> {
  constructor(firstStage: string, gate?: OpGate) {
    super(firstStage, treasuryErrorMessage, gate);
  }
}
