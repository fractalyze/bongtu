...
#
#   cd circuits && bash build/prove_all.sh                # all circuits
#   cd circuits && bash build/prove_all.sh transfer10     # just one (targeted regen)
#
...
cd "$(dirname "$0")/.."   # circuits/build -> circuits

# Overridable toolchain (same pattern as deploy/gates/e2e_m0.sh): CI / fresh machines
...
mkdir -p out verifiers fixtures/inputs
...
run_fixture() {
  ...
  $NODE "out/${circuit}_js/generate_witness.js" "out/${circuit}_js/$circuit.wasm" "fixtures/inputs/$fixture.json" "out/$fixture.wtns" \
