...
#
#   cd circuits && bash build_witness_so.sh                # the 3 GPU-served circuits
#   cd circuits && bash build_witness_so.sh disburse256    # just one
#
...
#   ZETO          the Zeto circuits checkout (same -l roots as prove_all.sh).
#
# witness_rt/ holds the bongtu-vendored r1cs-solver runtime MLIR (types /
...
for name in "${CIRCUITS[@]}"; do
  ...
  cat witness_rt/types.mlir witness_rt/helpers.mlir witness_rt/circom_runtime.mlir \
