...
`contracts/out/BongtuPool.sol/BongtuPool.json` — a **gitignored** forge build
...
cd contracts && forge build                      # writes out/BongtuPool.sol/BongtuPool.json
cd ..
node -e 'const fs=require("fs");const a=JSON.parse(fs.readFileSync("contracts/out/BongtuPool.sol/BongtuPool.json","utf8"));fs.writeFileSync("apps/indexer/abi/BongtuPool.abi.json",JSON.stringify({abi:a.abi},null,2)+"\n")'
```
