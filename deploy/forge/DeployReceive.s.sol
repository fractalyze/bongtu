...
import {ReceiveFactory} from "bongtu-src/ReceiveFactory.sol";

import {AddressBook, AddressRecord} from "./AddressBook.sol";

/// @title DeployReceive — the receive factory as an ADD-ON deploy.
///
/// One broadcast, one contract: `ReceiveFactory(bot)` next to the pool recorded
...
/// receive factory on a chain without a recorded `depositPrivModule` could
...
/// Rerun-guarded: a record that already carries a receive factory is refused —
...
contract DeployReceive is Script {
    ...
    function run() external returns (address factory) {
        ...
        require(r.receiveFactory == address(0), "receive factory already recorded for this chain");
        ...
        console2.log("== bongtu receive factory deploy ==");
