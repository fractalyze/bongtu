// The EVM half of the migration belt for the bjj key derivation (SPEC §6). The
// spending key is a pure function of the EIP-712 payload the wallet signs: if a
// wallet-stack migration (ethers -> viem, hand-rolled connect -> wagmi) changes ONE
// byte of that payload, every user's key silently rotates and their balance view is
// gone. This file owns the EIP-712 DIGEST half of the belt because this rail owns
// the typed-data seed (keyDerivationTypedData) and the signing edge; so it pins:
//
...
//   (2) that the REAL signing path turns a FIXED signature into the pinned bjj
//       identity — the wallet-to-KDF seam. The rail-free half — the pure
//       signature-to-identity pins (spend, view, KEM) — lives in the engine's own
//       suite, packages/client/test/deriveDeterminism.test.ts, because derive.ts's
//       byte plumbing is engine code and must be gated by `npm test -w
//       @bongtu/client` alone, with no viem in sight.
//
...
import { signKeyDerivation, type Connection } from "@bongtu/client-evm/connection";
