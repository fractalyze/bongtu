...
1. **Recipients** — a form (add rows `{compressed pubkey, amount}`) and/or an optional
   ...
   **compressed bjj pubkey** (`sdk/pubkey.ts` — a 32-byte hex string). Full ETH→bjj
...
- Decrypts each **transfer / disburse** authority envelope **locally** with the arbiter
  key (`src/lib/ledger.ts`, reusing `indexer/src/envelope.ts`) into "who received what /
...
A secondary **`GET /notes` lookup** exercises the signed read-auth flow (`sdk/eddsa.ts`).
...
```sh
# same env as prover-cli/CLAUDE.md: CUDA_VISIBLE_DEVICES=0, BONGTU_NODE_MODULES set,
...
src/
  ...
  lib/
    ...
    notesAuth.ts       signed GET /notes URL builder (sdk/eddsa)
