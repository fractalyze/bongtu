// The ONE icon module (locked visual language): every pictograph the UI needs is a
// thin wrapper over @remixicon/react, so screens never import the icon package
// directly and the whole set swaps in one place. Remix icons are fill-on-currentColor,
// so they inherit text color exactly like the retired hand-drawn stroke set. Text
// glyph characters and emoji stay banned. The single non-Remix exception is the
// MetaMask fox below — a third-party brand mark Remix does not carry.
...
/** The brand mark: an opened envelope (봉투) — user-picked ri-mail-open-line. */
export function EnvelopeLogo({ size = 28, className }: IconProps): ReactNode {
  return <RiMailOpenLine size={size} className={className} aria-hidden="true" />;
}
...
export function IconLink({ size = 16 }: IconProps): ReactNode {
  return <RiLinksLine size={size} aria-hidden="true" />;
}
...
export function IconSend({ size = 18 }: IconProps): ReactNode {
  return <RiSendPlane2Line size={size} aria-hidden="true" />;
}

// Direction semantics (locked): DEPOSIT moves money INTO the shielded pool =
// downward/inward arrow; WITHDRAW moves it OUT = the opposite. The two must read
// as opposites at a glance — a plus sign for deposit is retired.
export function IconDeposit({ size = 18 }: IconProps): ReactNode {
  return <RiDownload2Line size={size} aria-hidden="true" />;
}

export function IconWithdraw({ size = 18 }: IconProps): ReactNode {
  return <RiUpload2Line size={size} aria-hidden="true" />;
}
...
export function IconCopy({ size = 16 }: IconProps): ReactNode {
  return <RiFileCopyLine size={size} aria-hidden="true" />;
}

export function IconCheck({ size = 16 }: IconProps): ReactNode {
  return <RiCheckLine size={size} aria-hidden="true" />;
}
 ...
 *  official metamask-fox.svg from the MetaMask extension (viewBox 0 0 35 33). */
export function MetaMaskFox({ size = 18, className }: IconProps): ReactNode {
  return (
    <svg
      viewBox="0 0 35 33"
      width={size}
