...
`indexer`, and `prover-cli` **source directly**, so every commitment / nullifier /
...
The wallet shows the user's **compressed bjj pubkey** (`sdk/pubkey.ts` — a 32-byte hex
...
- **Primary — signed `GET /notes`.** Against an arbiter-mode indexer that has already
  ...
  control of its own key with an EdDSA-Poseidon read-auth signature (`sdk/eddsa.ts`,
...
src/
  ...
  lib/
    ...
    indexerClient.ts   /head /path /events /nullifiers + signed /notes URL (sdk/eddsa)
