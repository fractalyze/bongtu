...
// Writes solana/program/src/generated/vk_<snake_name>.rs (one per circuit)
//        solana/program/src/generated/fields.rs (BN254 moduli, shared)
//
...
/** The four consumer P2P circuits the Solana rail verifies (SOLR §2.3). */
const CIRCUITS: { name: string; snake: string; nPublic: number }[] = [
...
function emitCircuit(name: string, snake: string, wantPublics: number): void {
  const vkeyPath = join(REPO, "circuits", "out", `${name}.vkey.json`);
  ...
  const out = `// GENERATED by solana/scripts/gen_vk.ts from circuits/out/${name}.vkey.json
...
function emitFields(): void {
  const out = `// GENERATED by solana/scripts/gen_vk.ts — DO NOT EDIT.
...
function main(): void {
  emitFields();
  for (const c of CIRCUITS) emitCircuit(c.name, c.snake, c.nPublic);
}
