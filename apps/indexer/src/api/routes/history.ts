...
const AUTH_HEADER = {
  "x-bongtu-auth":
    "ENFORCED (bjj EdDSA-Poseidon sig over Poseidon(ownerPub.x,ownerPub.y,ts); |now-ts|<=300s replay window; SPEC §6b). Arbiter holds all activity by design.",
};
...
export const history: Route = {
  ...
  handle({ ix, query }) {
    ...
    // never leaks whether the owner has activity.
