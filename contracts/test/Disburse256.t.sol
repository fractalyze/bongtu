...
/// snarkjs-verified OK). Public layout (10, §5): [0..1]=ecdhPub [2]=disclosureHash
/// [3]=subtreeRoot [4]=nullifier [5]=root [6]=enabled [7]=nonce [8..9]=authorityPubKey.
///
...
/// seeding it reproduces the proof's membership root pub[5] on-chain (SpendCycle
...
contract Disburse256Test is Base {
    ...
    function _pub() internal view returns (uint[10] memory pub) {
    ...
    // authorityPublicKey (pub[8..9]) so the contract's storage-injected key
    ...
    function _seedInputAtLeaf0(BongtuPool pool) internal {
        ...
        assertEq(pool.root(), _pub()[5], "seed: pool.root() != public.json[5]");
    ...
    function testMembershipRootReproduced() public {
        ...
        // pub[6]=enabled=1, pub[8..9]=authorityPublicKey (matches arbiter key).
        uint[10] memory pub = _pub();
        assertEq(pub[6], 1, "enabled public signal must be 1");
        assertEq(pub[8], arbiterKey[0], "pub[8] != arbiter key x");
        assertEq(pub[9], arbiterKey[1], "pub[9] != arbiter key y");
    }
    ...
    function testDisburseFromPartialBlockMatchesOracle() public {
        ...
        assertTrue(pool.nullifierUsed(pub[4]), "partial-block disburse nullifier not marked");
