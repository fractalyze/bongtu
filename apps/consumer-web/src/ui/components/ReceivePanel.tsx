// The receive content (QR + full bongtu ID + copy) as ONE shared panel behind the
// Home modal. QR is rendered client-side from the qrcode lib into a data URL — no
// network. Deliberately TRIMMED against wallet-web's panel: the one-time deposit
// address surface (portal issuance) is enterprise-path and does not come along
// (issue #13's not-coming-along list), and the consumer Receive screen proper —
// pay-name registration of the v2 triple — is S6's; until then sharing the address
// IS the receive path, which is why the shell keeps this panel at all.
